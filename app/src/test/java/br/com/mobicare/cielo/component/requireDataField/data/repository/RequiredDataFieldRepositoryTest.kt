package br.com.mobicare.cielo.component.requireDataField.data.repository
...
class RequiredDataFieldRepositoryTest {

    private val remoteDataSource = mockk<RequiredDataFieldDataSourceImpl>()
    private val repository = RequiredDataFieldRepositoryImpl(remoteDataSource)
    ...
    fun `it should fetch update data calling remote data source only once`() = runBlocking {
        coEvery {
            remoteDataSource.postUpdateData(any(), any())
        } returns resultSuccess

        repository.postUpdateData(DEFAULT_OTP, requestUpdateData)

        coVerify(exactly = ONE) { remoteDataSource.postUpdateData(any(), any()) }
    }
    ...
    fun `it should return the success when update data required`() = runBlocking {
        ...
        val result = repository.postUpdateData(DEFAULT_OTP, requestUpdateData)
