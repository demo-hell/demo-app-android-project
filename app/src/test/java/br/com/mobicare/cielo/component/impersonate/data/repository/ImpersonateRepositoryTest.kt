...
class ImpersonateRepositoryTest {

    private val remoteDataSource = mockk<ImpersonateDataSource>()
    private val repository = ImpersonateRepositoryImpl(remoteDataSource)
    ...
    @Test
    fun `it should post impersonate remote data source only once`() = runBlocking {
        coEvery { remoteDataSource.postImpersonate(any(), any(), any()) } returns resultSuccess

        repository.postImpersonate(
            ImpersonateFactory.ec,
            TypeImpersonateEnum.HIERARCHY.name,
            ImpersonateFactory.impersonateRequest
        )

        coVerify(exactly = ONE) { remoteDataSource.postImpersonate(any(), any(), any()) }
    }
    ...
    fun `it should return access token when post impersonate`() = runBlocking {
        ...
        val result = repository.postImpersonate(
            ImpersonateFactory.ec,
            TypeImpersonateEnum.HIERARCHY.name,
            ImpersonateFactory.impersonateRequest
        )
    ...
    fun `it should return API error when post impersonate`() = runBlocking {
        coEvery { remoteDataSource.postImpersonate(any(), any(), any()) } returns resultError
    ...
    fun `it should return empty error when post impersonate`() = runBlocking {
        coEvery { remoteDataSource.postImpersonate(any(), any(), any()) } returns resultEmpty
