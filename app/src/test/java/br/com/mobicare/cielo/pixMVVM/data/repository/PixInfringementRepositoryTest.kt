...
class PixInfringementRepositoryTest {

    private val remoteDataSource = mockk<PixInfringementDataSource>()
    private val repository = PixInfringementRepositoryImpl(remoteDataSource)
    ...
    fun `it should fetch infringement calling remote data source only once`() = runBlocking {
        coEvery {
            remoteDataSource.getInfringement(any())
        } returns resultSuccessGetInfringement

        repository.getInfringement(PixInfringementFactory.idEndToEnd)

        coVerify(exactly = ONE) { remoteDataSource.getInfringement(any()) }
    }
    ...
    fun `it should return the infringement object`() = runBlocking {
        ...
        val result = repository.getInfringement(PixInfringementFactory.idEndToEnd)
    ...
    fun `it should create infringement calling remote data source only once`() = runBlocking {
        coEvery {
            remoteDataSource.postInfringement(any())
        } returns resultSuccessCreateInfringement

        repository.postInfringement(PixInfringementFactory.pixCreateInfringementRequest)

        coVerify(exactly = ONE) { remoteDataSource.postInfringement(any()) }
    }
