package br.com.mobicare.cielo.accessManager.presentation.batchProfileChange.adapter
...
import br.com.mobicare.cielo.databinding.ItemAccessManagerBatchChangeProfileBinding
import br.com.mobicare.cielo.extensions.addMaskCPForCNPJ
import br.com.mobicare.cielo.extensions.capitalizeWords
import br.com.mobicare.cielo.extensions.visible

class AccessManagerBatchChangeProfileAdapter(
    private val usersList: MutableList<AccessManagerUser>,
    val onUserDelete: (AccessManagerUser) -> Unit
) : RecyclerView.Adapter<AccessManagerBatchChangeProfileAdapter.AccessManagerBatchChangeProfileViewHolder>() {

    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): AccessManagerBatchChangeProfileViewHolder {
        val binding = ItemAccessManagerBatchChangeProfileBinding.inflate(
            LayoutInflater.from(parent.context),
            parent,
            false
        )
        return AccessManagerBatchChangeProfileViewHolder(binding)
    }

    override fun getItemCount() = usersList.size

    override fun onBindViewHolder(
        holder: AccessManagerBatchChangeProfileViewHolder,
        position: Int
    ) {
        holder.bind(usersList[position])
    }

    inner class AccessManagerBatchChangeProfileViewHolder(
        private val binding: ItemAccessManagerBatchChangeProfileBinding
        ...
        fun bind(accessManagerUser: AccessManagerUser) {
            binding.apply {
                ...
                icDelete.setOnClickListener {
                    onUserDelete(accessManagerUser)
                }
            }
        }
    }
