...
class SaveLocalInteractBannersOffersUseCase(private val repository: InteractBannerNewRepository) {

    suspend operator fun invoke(offers: List<HiringOffers>): CieloDataResult<Boolean> {
        return repository.saveLocalInteractBannersOffers(offers)
    }
}