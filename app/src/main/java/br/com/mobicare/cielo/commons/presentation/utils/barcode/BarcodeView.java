...
public class BarcodeView extends CameraPreview {
    ...
    private Integer barcodReadOKCount = 0;
                ...
                if (result != null) {
                    ...
                    if (barcodeFormat == BarcodeFormat.ITF) {
                        if (callback != null && decodeMode != DecodeMode.NONE) {
                            ...
                            //Verify if the same barcode was read 3 times in a row
                            if (decodeMode == DecodeMode.CONTINUOUS) {
                                if (result.getText().equals(lastBarcode)) {
                                    barcodReadOKCount++;
                                } else {
                                    barcodReadOKCount = 0;
                                }
                                lastBarcode = result.getText();
                                if (barcodReadOKCount >= 3) {
                                    callback.barcodeResult(result);
                                    stopDecoding();
                                }
                            }
