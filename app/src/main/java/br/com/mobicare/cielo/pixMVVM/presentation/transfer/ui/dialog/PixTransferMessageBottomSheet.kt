...
import br.com.mobicare.cielo.databinding.LayoutPixTransferReviewMessageBinding

class PixTransferMessageBottomSheet(
    ...
    fun show(fm: FragmentManager, tag: String? = null) {
        CieloContentBottomSheet.create(
            headerConfigurator = CieloBottomSheet.HeaderConfigurator(
                title = context.getString(R.string.pix_transfer_review_message_title)
            ),
            contentLayoutRes = R.layout.layout_pix_transfer_review_message,
            onContentViewCreated = ::buildContentView
        ).show(fm, tag)
    }

    private fun buildContentView(view: View, bs: CieloBottomSheet){
        LayoutPixTransferReviewMessageBinding.bind(view).apply {
