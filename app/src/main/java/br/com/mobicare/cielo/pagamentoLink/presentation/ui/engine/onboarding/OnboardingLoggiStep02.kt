...
import kotlinx.android.synthetic.main.onboarding_loggi_step_02.*
import org.jetbrains.anko.startActivity


class OnboardingLoggiStep02 : BaseFragment(), CieloNavigationListener {

    private var cieloNavigation: CieloNavigation? = null

    companion object {
        fun newInstance(bundle: Bundle) = OnboardingLoggiStep02().apply { this.arguments = bundle }
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.onboarding_loggi_step_02, container, false)
    }
    ...
    fun logicalSecondPage() {
        ...
        btnNextOnboarding.setOnClickListener {
            ...
            findNavController().navigate(OnboardingLoggiStep02Directions.actionOnboardingLoggiStep02ToOnboardingLoggiStep03())
        }

        txtEscolherOutraModalidade.setOnClickListener {
            gaSendSteps("escolher outra modalidade")
            findNavController().navigate(OnboardingLoggiStep02Directions.actionOnboardingLoggiStep02ToFormaEnvioFragment())
        }
    }
    ...
    private fun gaSendSteps(buttonType: String) {
        if (isAttached()) {
            Analytics.trackEvent(
                ...
                label = listOf(buttonType, "acompanhanhe", "clicado")
            )
        }
    }
}