package br.com.mobicare.cielo.idOnboarding.updateUser.p1Policy.benefits.custom
...
import br.com.mobicare.cielo.databinding.FragmentIdOnboardingCustomAccessBinding
...
import br.com.mobicare.cielo.idOnboarding.IDOnboardingFlowHandler.isLogin
...
class IDOnboardingCustomBenefitsInfoP1PolicyFragment : BaseFragment(),
    ...
    private var binding: FragmentIdOnboardingCustomAccessBinding? = null

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentIdOnboardingCustomAccessBinding.inflate(layoutInflater, container, false)
        return binding?.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        analytics.logIDScreenView(ANALYTICS_ID_SCREEN_VIEW_COMPLETE_PROFILE_SETUP + UserObj.MainRoleEnum.CUSTOM.description , this.javaClass)
    ...
    private fun setupView() {
        ...
        binding?.apply{
            ...
            userStatus.p1Flow?.deadlineRemainingDays?.let {
                if (it <= ZERO)
                    btSendLater.gone()
            }
            ...
            includePersonalInfo.apply {
                tvRoleName.text =
                    getString(R.string.access_manager_role_group_card_description_custom_title)
                ...
                tvAccessExplanation.fromHtml(
                    R.string.id_onboarding_p1_custom_access_can_title
                )
        ...
        binding?.apply {
            btSendNow.setOnClickListener {
                ...
                analytics.logIDOnClickSendPictures(
                    ...
                    UserObj.MainRoleEnum.CUSTOM.description
                )
                analyticsGA.logIDSendPicturesNowSignUp(ANALYTICS_ID_SCREEN_VIEW_SEND_PICTURES)
                goToSendPhotosNow()
            }
            ...
            btSendLater.setOnClickListener {
                ...
                analyticsGA.logIDSendPicturesLaterClick(ANALYTICS_ID_SCREEN_VIEW_SEND_PICTURES)
                goToSendPhotosLater()
            }

            btGoToHelpCenter.setOnClickListener {
                it.isEnabled = false
                analytics.logIDOnClickHelpCenter(UserObj.MainRoleEnum.CUSTOM.description)
    ...
    private fun goToSendPhotosNow() {
        findNavController().navigate(
            when (IDOnboardingFlowHandler.checkpointP2) {
                IDOCheckpointP2.DOCUMENT_PHOTO_UPLOADED -> IDOnboardingCustomBenefitsInfoP1PolicyFragmentDirections.actionIdOnboardingCustomBenefitsFragmentToIdOnboardingPicturesSelfieGuideFragment()
                else -> IDOnboardingCustomBenefitsInfoP1PolicyFragmentDirections.actionIdOnboardingCustomBenefitsFragmentToIdOnboardingPicturesSelectDocFragment()
            }
        )
    }

    private fun goToSendPhotosLater() {
        findNavController().navigate(IDOnboardingCustomBenefitsInfoP1PolicyFragmentDirections.actionIdOnboardingCustomBenefitsFragmentToIdOnboardingPicturesLaterFragment(
            UserObj.MainRoleEnum.CUSTOM.description
        ))
    }
    ...
    private fun setupStepRestartBottomSheet() {
        if (userStatus.onboardingStatus?.userStatus?.restartP2 == true) {
            ...
            IDOnboardingFlowHandler.showCustomBottomSheet(
                activity = this@IDOnboardingCustomBenefitsInfoP1PolicyFragment.activity,
