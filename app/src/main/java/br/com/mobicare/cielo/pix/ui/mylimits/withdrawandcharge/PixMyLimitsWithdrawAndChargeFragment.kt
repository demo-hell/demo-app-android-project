...
import br.com.mobicare.cielo.pix.ui.mylimits.withdrawandcharge.helpers.PixMyLimitsWithdrawAndChargeTypeViewSelector
...
class PixMyLimitsWithdrawAndChargeFragment : BaseFragment(), CieloNavigationListener,
    PixMyLimitsWithdrawAndChargeContract.View, AllowMeContract.View {

    private val presenter: PixMyLimitsWithdrawAndChargePresenter by inject {
        parametersOf(this)
    }
    ...
    private var binding: FragmentPixMyLimitsWithdrawAndChargeBinding? = null
    private var navigation: CieloNavigation? = null
    private var limitsResponse: PixMyLimitsResponse? = null
    private var isAnimation = true

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View = FragmentPixMyLimitsWithdrawAndChargeBinding.inflate(
    ...
    private fun setupNavigation() {
        if (requireActivity() is CieloNavigation) {
            ...
            navigation?.setTextToolbar(getString(R.string.text_toolbar_pix_my_limits_withdraw_and_change))
    ...
    private fun bottomSheetErrorUpdateLimit(error: ErrorMessage?) {
        ...
        if (error?.code != HttpURLConnection.HTTP_FORBIDDEN.toString() || error.errorCode.contains(
            ...
            bottomSheetGenericFlui(
                ...
                getString(R.string.text_pix_generic_error_title),
    ...
    override fun onShowMyLimits(limitsResponse: PixMyLimitsResponse) {
        limitsResponse.limits?.let { limits ->
            ...
            for (limit in limits) {
                ...
                binding?.let {
                    PixMyLimitsWithdrawAndChargeTypeViewSelector(
                        limit = limit,
                        binding = it,
    ...
    private fun toBack() {
        findNavController().navigate(PixMyLimitsWithdrawAndChargeFragmentDirections.actionPixMylimitsWithdrawAndChargeFragmentToPixMyLimitsFragment())
    }
