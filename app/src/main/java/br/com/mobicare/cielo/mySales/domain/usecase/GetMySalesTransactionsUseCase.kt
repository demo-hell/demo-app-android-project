...
class GetMySalesTransactionsUseCase(private val repository: MySalesRemoteRepository) {
    ...
    suspend operator fun invoke(params: GetSalesDataParams): CieloDataResult<SummarySalesBO> {
        ...
        repository.getMySalesTransactions(params)
            .onSuccess {
                result = if (it.items.isEmpty().not()) {
