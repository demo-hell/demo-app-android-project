...
class GetCardBrandsUseCase(private val repository: MySalesFiltersRemoteRepository) {
    suspend operator fun invoke(params: GetBrandsSalesFiltersParams):
            CieloDataResult<CardBrandsBO> {
        return repository.getFilteredCardBrands(params)
    }
}