package br.com.mobicare.cielo.pix.ui.extract.home.tabs.pageScheduled
...
import br.com.mobicare.cielo.databinding.FragmentPixExtractPageScheduledTransactionBinding
...
class PixExtractPageScheduledTransactionFragment : BaseFragment(),
    ...
    private var _binding: FragmentPixExtractPageScheduledTransactionBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding =
            FragmentPixExtractPageScheduledTransactionBinding.inflate(inflater, container, false)
        return binding?.root
    }
        ...
        safeRun {
            presenter.run {
                if (alreadyLoaded.not())
                    getExtract(true, ReceiptsTab.SCHEDULES, null)
            }
        }
    }
    ...
    private fun setupInformative() {
        binding?.includeInformation?.apply {
            tvTitleFooter.text =
                getString(R.string.text_pix_extract_title_information_page_scheduled)
            tvMessageFooter.text =
                getString(R.string.text_pix_extract_message_information_page_scheduled)
        }
    }
    ...
    private fun initializeRecyclerView(extract: PixExtractResponse?) {
        safeRun {
                            ...
                            override fun onScrollStateChanged(
                                ...
                                binding?.recyclerExtractPageAll?.post {
                                    if (recyclerView.canScrollVertically(ONE)
                                        ...
                                        oldState = newState
                                        showLoadingMore()
                                        presenter.getExtract(false, ReceiptsTab.SCHEDULES, null)
                                    }
                                }
                            }
                        })
                    ...
                    this.onClick =
                        object : BottomSheetFluiGenericFragment.OnClickButtonsOptionsListener {
                            ...
                            override fun onBtnSecond(dialog: Dialog) {
                                dialog.dismiss()
                                presenter.getExtract(isFirstPage, ReceiptsTab.SCHEDULES, null)
                            }
    ...
    override fun showExtract(extract: PixExtractResponse?, isFirstPage: Boolean, isFilter: Boolean) {
        safeRun {
            initializeRecyclerView(extract)
            extract?.let {
